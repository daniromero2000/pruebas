...
<div class="container-fluid col-md-4">
    <div class="clearfix container-fluid main-content">
        ...
        <a href=" {{ route('admin.issues.edit', [$issue->id]) }}">Edit</a>
        <p>Description: {{$issue->description}}</p>
        <p>Status: {{Modules\Development\Entities\Issues\IssueStatus::find($issue->status_id)->label}}</p>
        <?php $issueDeadline = $issue->deadline;?>
        @if($issueDeadline)
        Deadline: {{$issueDeadline->year}}-{{$issueDeadline->month}}-{{$issueDeadline->day}}
        @endif
        <p>Project: {{Modules\Development\Entities\Projects\Project::find($issue->project_id)->name}}</p>
    </div>
